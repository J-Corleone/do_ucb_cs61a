...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    cur_palyer = player_score % 10
    opp_player = opponent_score % 100 // 10
    calc = abs(cur_palyer - opp_player) * 3
    return max(1, calc)
    # END PROBLEM 2
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    res, k = 0, 0
    for i in range(10):
        score = make_averaged(roll_dice, times_called)(i+1, dice)
        if score > res:
            res, k = score, i+1
    return k
    # END PROBLEM 9
